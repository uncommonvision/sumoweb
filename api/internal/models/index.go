...
func (m Match) Key() string {
	return fmt.Sprintf("%s-vs-%s", m.East.ID, m.West.ID)
}

func (m Match) HasChangedFrom(old Match) bool {
	if m.East.Result != old.East.Result ||
		m.East.Won != old.East.Won ||
		m.East.Lost != old.East.Lost ||
		m.East.RestJP != old.East.RestJP ||
		m.East.RestEN != old.East.RestEN {
		return true
	}

	if m.West.Result != old.West.Result ||
		m.West.Won != old.West.Won ||
		m.West.Lost != old.West.Lost ||
		m.West.RestJP != old.West.RestJP ||
		m.West.RestEN != old.West.RestEN {
		return true
	}

	if m.JudgeID != old.JudgeID || m.TechnicID != old.TechnicID {
		return true
	}
